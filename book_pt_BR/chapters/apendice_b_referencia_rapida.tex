...
\midrule
\texttt{rnormal()}       & $U(0,1)$ — uniforme, \emph{não} normal$^*$ \\
\texttt{runiform()}      & $U(0,1)$ — alias de \texttt{rnormal()} \\
...
{\footnotesize $^*$ \textbf{Atenção:} \texttt{rnormal()} em Hayashi gera $U(0,1)$.
  Para simular $N(0,1)$, use a transformação $\Phi^{-1}(u)$ ou a soma de 12 variáveis
  uniformes (teorema do limite central). Em versões futuras, \texttt{rnormal()}
  poderá ser corrigida para $N(0,1)$.}
