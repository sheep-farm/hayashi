...
Todos exigem um indicador de tratamento `d` e um conjunto de covariáveis
`x`. Eles não substituem um desenho de pesquisa claro, mas podem ajudar
...
`bsc` implementa uma versão bayesiana do estimador de controle
...
`dml_crossfit` estima o ATE no modelo parcialmente linear
...
Todos esses comandos são \textit{experimentais}. A validação contra
