...
`bayes_lm` implementa regressão linear bayesiana com priori conjugada
...
onde $v$ é o ruído e $u$ é a ineficiência. `sfa_production` estima
...
`bayes_sfa_production` e `bayes_sfa_cost` implementam um amostrador de
...
\begin{itemize}
\item \textbf{Inferência padrão com incerteza:} `bayes_lm`
\item \textbf{Eficiência de produção:} `sfa_production`
\item \textbf{Eficiência de custo:} `sfa_cost`
\item \textbf{SFA em amostras pequenas ou posteriori completa:} `bayes_sfa_production`
\end{itemize}
