...
\texttt{predict}, \texttt{glance} e \texttt{tidy}.
...
Use \texttt{predict(df, yhat = m\_rf)} para obter valores ajustados em um
DataFrame.
...
O comando \texttt{mlp} ajusta uma rede feed-forward com uma camada oculta,
...
Para séries temporais, \texttt{lstm} e \texttt{transformer} usam janelas de
...
qualidade preditiva é sensível a convenções. São experimentais; para usar
\texttt{lstm} e \texttt{transformer}, compile o Hayashi com \texttt{--features experimental}.
...
objeto compatível com \texttt{predict} e \texttt{glance}. Erros padrão geralmente não
