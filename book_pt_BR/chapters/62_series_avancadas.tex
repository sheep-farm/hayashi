...
Em \texttt{tvp} o vetor de coeficientes é um passeio aleatório observado com
ruído:
...
\texttt{tvp} estima as variâncias de $\varepsilon_t$ e $\eta_t$ por MV e
...
Use \texttt{tvp\_var} para uma VAR completa com coeficientes variantes no tempo.

\section{VAR de limiar e de mudança de regime Markov}

\texttt{setar} permite dois regimes AR divididos por um valor de limiar da
...
\texttt{msvar} permite que os coeficientes mudem de acordo com uma cadeia
latente de Markov.
...
\texttt{nardl} decompõe um regressor em somas parciais positivas e negativas,
...
\texttt{pstr} faz o mesmo em painel, com coeficientes que mudam suavemente de
...
\texttt{fcoef} estima coeficientes que variam suavemente com uma variável
...
\texttt{bvar} estima uma VAR com priori de Minnesota, encolhendo defasagens
...
\texttt{mfvar} combina variáveis de baixa e alta frequência usando agregação
...
\texttt{fcoef}, \texttt{bvar} e \texttt{mfvar} são experimentais e
exigem \texttt{--features experimental}.

\section{VAR em painel}

\texttt{pvar} estima uma VAR em painel usando GMM estilo Arellano-Bond:
