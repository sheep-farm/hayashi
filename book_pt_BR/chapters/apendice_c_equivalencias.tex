...
\noindent\textbf{Nota sobre \texttt{rnormal()}:} em Hayashi, gera $U(0,1)$, \emph{não} $N(0,1)$.
Ao replicar DGPs em outras plataformas, substitua por \texttt{runiform()} (Stata),
\texttt{runif()} (R) ou \texttt{np.random.uniform()} (Python).
