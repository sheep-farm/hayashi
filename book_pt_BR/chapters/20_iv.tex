...
\label{cap:iv}
% ── índice ──────────────────────────────────────────────────────
\index{variáveis instrumentais|textbf}
\index{IV|see{variáveis instrumentais}}
\index{iv@\texttt{iv()}|textbf}
\index{endogeneidade|textbf}
\index{dois estágios, mínimos quadrados de}
\index{2SLS|see{dois estágios, mínimos quadrados de}}
\index{condição de relevância|textbf}
\index{condição de exclusão|textbf}
\index{primeiro estágio}
...
Sejam $z$ e $v$ variáveis aleatórias independentes $\sim \mathcal{N}(0,1)$.
Defina $\varepsilon$ como o \emph{resíduo} da projeção de $v$ sobre $[1,\;z]$:
\[
...
Por construção $\sum \varepsilon_i = 0$ e $\sum z_i\varepsilon_i = 0$, ou seja,
$Z'\varepsilon = 0$ \emph{exatamente} na amostra. Então:
...
O erro estrutural $0{,}7\varepsilon$ cria endogeneidade ($\mathrm{Cov}(x,0{,}7\varepsilon) \neq 0$),
mas como $Z'\varepsilon = 0$ exatamente, as condições de momento do IV são
satisfeitas sem ruído amostral. Resultado: IV recupera $(\beta_0, \beta_1)$
com precisão de máquina — o mesmo que OLS fez sem ruído no Capítulo~\ref{cap:ols}.
...
O IV recupera $(\hat\beta_0, \hat\beta_1) = (1{,}5000,\; 2{,}0000)$ com precisão
de máquina. O OLS superestima $\beta_1$ em $+0{,}43$ (2.4303 vs.\ 2.0000),
consistente com o viés assintótico previsto de $+0{,}43$.
O $R^2$ não é reportado para IV pois não tem interpretação padrão sob endogeneidade.
...
\subsection{Transformações e interações nas fórmulas}
\label{subsec:iv-formula-avancada}

As mesmas transformações e interações disponíveis no OLS
(Seção~\ref{subsec:formula-transformacoes}--\ref{subsec:formula-I})
funcionam igualmente nas fórmulas estrutural e de instrumentos do IV:

\begin{lstlisting}[caption={IV com transformações e interações}]
// regressor endógeno transformado
iv(Y ~ log(X2), ~ Z, df)

// interação de transformações na equação estrutural
iv(Y ~ log(K) + log(L) + log(K):log(L), ~ Z1 + Z2, df)

// instrumento transformado
iv(Y ~ X2, ~ log(Z), df)
\end{lstlisting}
...
\section{Testes de pós-estimação de IV}

\subsection{Sargan / Hansen J — sobreidentificação}

Quando há mais instrumentos que variáveis endógenas (sobreidentificação),
testa-se a exogeneidade dos instrumentos:

\begin{lstlisting}
estat_overid(y ~ x1 + x_endog, ~ z1 + z2, df)
\end{lstlisting}

$H_0$: instrumentos são exógenos (válidos). A estatística de Sargan é
$n \cdot R^2$ da regressão dos resíduos IV sobre os instrumentos,
distribuída como $\chi^2(L - K)$, onde $L$ é o número de instrumentos e
$K$ o de regressores. Rejeitar $H_0$ indica que os instrumentos podem
ser inválidos. Não se aplica quando o modelo é exatamente identificado
($L = K$).

\subsection{Durbin-Wu-Hausman — endogeneidade}

Testa se a variável suspeita é realmente endógena, comparando OLS e IV:

\begin{lstlisting}
estat_endog(y ~ x1 + x_endog, ~ z1 + z2, df)
\end{lstlisting}

$H_0$: regressores são exógenos (OLS é consistente). Usa a abordagem de
regressão augmentada: adiciona os resíduos do primeiro estágio à equação
estrutural e testa sua significância via $F$. Se rejeitar $H_0$, IV é
necessário; se não rejeitar, OLS é consistente e preferível (mais
eficiente).
