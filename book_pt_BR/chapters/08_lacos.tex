...
\texttt{return} explícito) é coletado em uma lista, que é armazenada de
volta na \textbf{variável de iteração} após todas as threads terminarem.

\begin{lstlisting}[caption={parallel for básico}]
parallel for t in tickers {
  let sub = filter(df, ticker == t)
  nrow(sub)
}
// t agora contém uma lista de valores nrow, um por ticker
\end{lstlisting}
...
Cada thread recebe seu próprio interpretador com um snapshot do ambiente
externo (apenas valores send-safe são capturados). Use \texttt{return nil}
para pular uma iteração; entradas \texttt{nil} são mantidas na lista de
resultados.

Combine com \texttt{rbind()} para agregar DataFrames por iteração:
...
let todos = rbind(i)   // i contém a lista de DataFrames
\end{lstlisting}
