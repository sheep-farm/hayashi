...
ambiente externo (apenas valores send-safe são capturados). O
auto-display é suprimido dentro das threads --- apenas chamadas
explícitas a \texttt{print()} produzem saída. Isso tem consequências
importantes:
...
\begin{itemize}
  ...
  \item \textbf{Auto-display é suprimido}: a última expressão do corpo é
    coletada como resultado, não impressa. Use \texttt{print()}
    explicitamente se precisar de saída de iterações individuais.
\end{itemize}
