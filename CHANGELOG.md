...
- The strict validation audit records 226 cases across 115 implemented estimators: 200 `pass`, 9 `fail`, 2 `blocked`, and 15 `not-supported`. The failed and blocked cases are tracked in #137, #138, and #139; they must not be represented as passing release evidence. Several families remain thinly or convention-sensitive validated; see `KNOWN_GAPS.md` for details.
...
- Estimators with single-language or algorithm-matched references: `clogit` (R), `kalman` (R), `tobit` (R), `cpoisson` (Python), `elasticnet` (Python), `feiv` (Python), `garch` (Python), `lpdid` (Python), `three_sls` (Python), `varma` (Python).
...
- **`sv(df, var [, iter=100])`**: Stochastic Volatility (Taylor 1986, Harvey-Ruiz-Shephard 1994). Latent log-volatility AR(1) process: yₜ = exp(hₜ/2)·εₜ, hₜ = μ + φ·(hₜ₋₁ − μ) + ηₜ. Quasi-MCMC via single-site Metropolis-Hastings on hₜ. Reports μ, φ (persistence), σ_η (vol-of-vol), conditional volatility path. Alias: `stochastic_vol`.
- **`fapanel(y ~ x1 + x2, df, aux="aux_df", id="entity", period="period" [, factors=2])`**: Factor-augmented panel regression (Giannone-Lenza-Primiceri style). Two-step: (1) PCA factors from auxiliary panel via power iteration with deflation, (2) FE (within) regression on traditional regressors + factors. Reports β (regressors), γ (factors), R². Alias: `fa_panel`.
- **`hawkes(df, time_var [, T=100])`**: Hawkes self-exciting point process (Hawkes 1971). Intensity λ(t) = μ + Σ α·exp(−β·(t−tᵢ)). Grid search MLE over (μ, α, β) with stationarity constraint α/β < 1. Reports branching ratio η = α/β, intensity at event times, AIC/BIC.
- **`rf(y ~ x1 + x2, df [, trees=100, depth=10])`**: Random Forest regression (Breiman 2001). Ensemble of CART decision trees with bootstrap sampling (bagging) + random feature subsampling (mtry = √k). MSE-based split criterion. Reports feature importance (impurity decrease), in-sample R², OOB R². Alias: `random_forest`.
- **`gbm(y ~ x1 + x2, df [, trees=100, lr=0.1, depth=3, subsample=1.0])`**: Gradient Boosting regression (Friedman 2001). Sequential additive model of shallow regression trees fit to pseudo-residuals. Shrinkage (lr) for regularization. Stochastic subsampling optional. Reports feature importance, in-sample R². Alias: `gradient_boosting`.
- **`mlp(y ~ x1 + x2, df [, hidden=10, lr=0.01, epochs=200])`**: Multilayer Perceptron (Rumelhart-Hinton-Williams 1986). Single hidden layer, sigmoid activation. Backpropagation with momentum (0.9). Xavier initialization. Standardized internally. Reports weight statistics, final MSE, R². Alias: `neural_net`.
- **`synthdid(df, y_var, treated_var, treatment_period, unit="col", period="col")`**: Synthetic DiD (Arkhangelsky et al. 2021). Combines synthetic control + DiD. Unit weights via OLS on pre-period outcomes. Time weights on pre-periods. Placebo/permutation SE. Reports ATT, unit/time weights, synthetic control path. Alias: `synthetic_did`.
- **`cuped(y ~ x, df, treated="treated_var")`**: CUPED (Deng-Xu-Kohavi-Walker 2013). Adjusts A/B test outcome using pre-treatment covariate: Y_adj = Y − θ·(X − X̄), θ = Cov(Y,X)/Var(X). Reduces variance, tighter CIs. Reports adjusted vs unadjusted effect, variance reduction, 95% CI.
- **`qrf(y ~ x1 + x2, df [, quantiles="0.1,0.5,0.9", trees=100, depth=10])`**: Quantile Regression Forest (Meinshausen 2006). Stores all leaf responses instead of just the mean. Computes weighted empirical CDF across all trees for conditional quantile estimation. Reports OOB R² (median), feature importance, quantile predictions. Alias: `quantile_forest`.
- **`xgboost(y ~ x1 + x2, df [, trees=100, lr=0.3, depth=6, lambda=1.0, alpha=0.0, gamma=0.0, subsample=1.0, colsample=1.0])`**: XGBoost (Chen & Guestrin 2016). L1/L2 regularized gradient boosting with Newton (second-order) splits. gamma=leaf penalty, lambda=L2, alpha=L1. Row/column subsampling. Feature importance via cumulative gain. Alias: `xgb`.
- **`dml_crossfit(y ~ d, df, x="x1,x2" [, folds=5])`**: Double ML with cross-fitting (Chernozhukov et al. 2018). Partially linear model: Y = θ·D + g(X) + ε. K-fold cross-fitting: nuisance functions g(X) and m(X) estimated on training folds, residuals computed on held-out fold. θ via OLS of Y-residuals on D-residuals. Reports causal effect, SE, CI, nuisance MSE. Alias: `dml_cf`.
- **`bsc(df, y_var, control_vars, treatment_period [, prior=1.0])`**: Bayesian Synthetic Control (Brodersen et al. 2015, Scott 2019). Conjugate Normal-Inverse-Gamma posterior. Ridge-like prior on weights. Weaker prior on treatment effect. Reports τ (posterior mean), credible interval (t-distribution), weights, counterfactual path, cumulative effect. Alias: `bayesian_sc`.
- **`lstm(df, var [, hidden=10, seqlen=10, lr=0.01, epochs=100, forecast=5])`**: LSTM recurrent neural network (Hochreiter & Schmidhuber 1997). Single-layer LSTM with forget/input/candidate/output gates. Truncated BPTT training. Standardized internally. Reports fitted values, multi-step forecast, MSE, R², final hidden/cell state.
- **`causalforest(y ~ treated, df, x="x1,x2" [, trees=100, depth=5])`**: Causal Forest (Wager & Athey 2018). Honest causal trees for heterogeneous treatment effects (HTE). Split sample: one half for splitting, one half for leaf effect estimation. Splitting criterion maximizes variance of treatment effects across leaves. Reports ATE, CATE distribution, feature importance, heterogeneity SD. Alias: `causal_forest`.
- **`grf(y ~ treated, df, x="x1,x2" [, trees=100, depth=5])`**: Generalized Random Forest (Athey, Tibshirani & Wager 2019). Doubly-robust CATE with local centering. AIPW estimator for ATE. Pseudo-outcome regression for CATE. Propensity and outcome regression via OLS. Reports ATE, CATE, CI, propensity, outcome regression. Alias: `generalized_rf`.
- **`conformal(y ~ x1 + x2, df [, alpha=0.1, calib=0.3])`**: Conformal Prediction (Vovk, Gammerman & Shafer 2005). Distribution-free prediction intervals with finite-sample coverage guarantee. Split conformal with OLS base predictor. Reports prediction intervals, conformal quantile, empirical coverage, calibration scores. Alias: `conformal_pred`.
- **`transformer(df, var [, d_model=8, seqlen=10, lr=0.001, epochs=100, forecast=5])`**: Transformer (Vaswani et al. 2017). Single-head, single-layer encoder for time series. Scaled dot-product attention. Sinusoidal positional encoding. ReLU feed-forward. Reports fitted values, multi-step forecast, MSE, R². Alias: `transformer_ts`.
- **`dr_learner(y ~ treated, df, x="x1,x2" [, folds=3])`**: DR-Learner (Kennedy 2023). Doubly-robust CATE via pseudo-outcome regression. Cross-fitting for nuisance functions. DR pseudo-outcome: AIPW-based. Reports ATE, CATE, CI, propensity, outcome regression, CATE coefficients. Alias: `drlearner`.
- **`bart(y ~ x1 + x2, df [, trees=20, depth=3, iter=100, burnin=20])`**: BART (Chipman, George & McCulloch 2010). Bayesian Additive Regression Trees. Sum-of-trees with Bayesian shrinkage. Gibbs sampling for σ². Variable inclusion proportions. Reports fitted values, posterior σ², R², MSE. Alias: `bayesian_trees`.
- **`gp(y ~ x1 + x2, df)`**: Gaussian Process Regression (Rasmussen & Williams 2006). Squared Exponential (RBF) kernel. Grid search MLE for hyperparameters (length scale, signal variance, noise). Reports fitted values with uncertainty, log marginal likelihood. Alias: `gaussian_process`.
- **`tmle(y ~ treated, df, w="c1,c2")`**: TMLE (van der Laan & Rubin 2006). Targeted Maximum Likelihood Estimation. Doubly-robust ATE via targeting step. Initial Q(W) and g(W) via OLS. Clever covariate H. Fluctuation epsilon. EIF-based SE. Reports ATE, SE, CI, epsilon, initial ATE, propensity, Q*, EIF.
- **`orf(y ~ treated, df, x="x1,x2", w="c1,c2" [, trees=50, depth=5])`**: Orthogonal Random Forest (Oprescu, Syrgkanis & Wu 2019). Doubly-robust CATE via orthogonalization + forest. Residualize T and Y w.r.t. confounders W. Forest on residualized signals. Reports ATE, CATE, CI, feature importance. Alias: `orthogonal_forest`.
- **`spectral(df, x="x1,x2", k=3 [, sigma=auto])`**: Spectral Clustering (Ng, Jordan & Weiss 2002). Clustering via eigenstructure of similarity graph. Gaussian kernel affinity. Normalized graph Laplacian. K smallest eigenvectors. K-means in eigenspace. Reports labels, eigenvalues, inertia. Alias: `spectral_clustering`.
- **`isotonic(y ~ x, df [, decreasing])`**: Isotonic Regression (Barlow et al. 1972). Monotone regression via Pool Adjacent Violators Algorithm (PAVA). Non-decreasing by default; use `decreasing` for non-increasing. Reports fitted values, step function, R², MSE. Alias: `isotonic_reg`.
- **`causal_impact(df, y_var, controls="c1,c2", period=N)`**: Causal Impact (Brodersen, Gallus, Henderson & Orban 2015). Bayesian structural time series for counterfactual prediction. OLS on pre-treatment + local level state. Reports avg effect, cumulative effect, CIs, P(effect>0), counterfactual path. Alias: `causalimpact`.
- **`mice_chained(df, vars="x1,x2,x3" [, m=5, iter=10])`**: MICE (van Buuren & Groothuis-Oudshoorn 2011). Multiple imputation by chained equations. Iterative regression imputation. Rubin's rules for pooling. Reports imputed data, pooled mean, variance, FMI. Alias: `mice_eq`.
- **`kmeans(df, x="x1,x2", k=3)`**: K-Means Clustering (MacQueen 1967, Lloyd 1982). K-means++ initialization. Lloyd's algorithm. Reports labels, centroids, inertia, between/total SS, % explained. Alias: `k_means`.
- **`bayes_lm(y ~ x1 + x2, df)`**: Bayesian Linear Regression with conjugate Normal-Inverse-Gamma prior. Closed-form posterior. Weak default prior. Reports posterior mean, SD, 95% credible intervals, P(coef>0), σ², log marginal likelihood, R². Alias: `bayesian_lm`.
- **`dbscan(df, x="x1,x2", eps=1.0, minpts=5)`**: DBSCAN (Ester, Kriegel, Sander & Xu 1996). Density-based clustering. Groups points in high-density regions, marks low-density as noise. No need to specify k. Finds arbitrary-shaped clusters. Reports labels, clusters, noise points, core points. Alias: `dbscan_clust`.
- **`gmm_clust(df, x="x1,x2", k=3 [, iter=100, tol=1e-6])`**: GMM Clustering (Dempster, Laird & Rubin 1977). Gaussian mixture via EM. Probabilistic cluster assignment. Reports labels, means, covariances, weights, log-likelihood, BIC, AIC. Alias: `gmm_clustering`.
- **`reg_path(y ~ x1 + x2, df, type="lasso" [, alpha=1.0, nlambda=50])`**: Regularization Path (Efron & Hastie 2004; Zou & Hastie 2005). Ridge/Lasso/ElasticNet coefficient paths. Coordinate descent. BIC-based optimal lambda selection. Reports paths, optimal lambda, selected variables. Alias: `regpath`.
- **`qrf_inf(y ~ x1 + x2, df, q="0.1,0.5,0.9" [, boot=100, trees=100, depth=10, conf=0.95])`**: QRF Inference (Meinshausen 2006; Athey, Tibshirani & Wager 2019). Quantile Regression Forest with bootstrap CIs. Pointwise bands. Coverage diagnostics. Reports quantile estimates, CIs, OOB R², feature importance. Alias: `qrf_inference`.
- **`hclust(df, x="x1,x2" [, linkage="ward", cut=auto])`**: Hierarchical Clustering (Ward 1963; Johnson 1967). Agglomerative. Linkage: ward, single, complete, average. Builds dendrogram. Reports merges, labels at cut, cophenetic correlation. Alias: `hierarchical`.
- **`tsne(df, x="x1,x2" [, perplexity=30, iter=500, lr=200])`**: t-SNE (van der Maaten & Hinton 2008). Nonlinear dimensionality reduction for visualization. Gaussian high-D affinities. Student-t low-D. KL divergence minimization. Reports 2D embedding, KL divergence, cost history. Alias: `t_sne`.
- **`umap(df, x="x1,x2" [, neighbors=15, iter=300])`**: UMAP (McInnes, Healy & Melville 2018). Uniform Manifold Approximation and Projection. k-NN graph + fuzzy simplicial set. Cross-entropy optimization. Preserves local and global structure. Reports embedding, loss history.
- **`biplot(df, x="x1,x2,x3" [, type="symmetric"])`**: PCA Biplot (Gabriel 1971). Simultaneous visualization of observations (scores) and variables (loadings). Types: form, covariance, symmetric. Reports scores, loadings, explained variance, ASCII biplot. Alias: `pca_biplot`.
- **`hausman_robust(fe_model, re_model)`**: Robust Hausman Test (Cameron-Trivedi 2005; Wooldridge 2010). Compares FE vs RE using robust covariance of coefficient differences. Valid under heteroskedasticity and clustering. Reports Chi2, df, p-value, beta differences. Alias: `hausman_r`.
- **`ftest_robust(model [, vars="x1,x2"])`**: Robust F-Test (Wooldridge 2010). Joint significance test using Wald statistic with robust covariance. Tests H0: all specified coefficients are zero. Default: all slopes. Reports Wald Chi2, F-stat, p-values (F and Chi2). Alias: `f_robust`.
- **`tidy()` extended to 18 more model types**: now supports VAR, VECM, SystemGMM, FE2SLS, PCSE, PanelGLS, GLSAR, RecursiveLS, Cox, Conditional, GAM, Mixed, ZeroInflated, AutoReg, ARDL, DiD, Threshold, ETS, LocalLevel — in addition to all previously supported types.
- **`glance()` extended to 18 more model types**: now supports VAR, VECM, SystemGMM, FE2SLS, PCSE, PanelGLS, GLSAR, RecursiveLS, Cox, Conditional, GAM, Mixed, ZeroInflated, AutoReg, ARDL, DiD, Threshold, ETS, LocalLevel — in addition to all previously supported types.
