...
use odbc_api::{Environment, ConnectionOptions, Cursor, buffers::TextRowSet, ResultSetMetadata};
...
pub fn load_odbc(conn_str: &str, query: &str) -> Result<(DataFrame, usize)> {
    ...
    let mut stmt = conn.execute(query, (), None)
    ...
    let n_cols = stmt.num_result_cols()
        .map_err(|e| HayashiError::Runtime(format!("ODBC metadata error: {e}")))? as usize;

    let col_names: Vec<String> = (1..=n_cols as u16).map(|i| {
        stmt.col_name(i)
    ...
    let mut cursor = stmt.bind_buffer(&mut buffers)
        ...
        .map_err(|e| HayashiError::Runtime(format!("ODBC fetch error: {e}")))?
    {
        ...
        for row_idx in 0..rows_in_batch {
            for col_idx in 0..n_cols {
                let val = batch.at(col_idx, row_idx)
    ...
    for (i, name) in col_names.iter().enumerate() {
        ...
        if is_numeric_column(vals) {
            let floats: Vec<f64> = vals.iter().map(|s| {
                if s.is_empty() { f64::NAN } else { s.parse::<f64>().unwrap_or(f64::NAN) }
            }).collect();
