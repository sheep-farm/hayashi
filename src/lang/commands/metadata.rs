...
impl CommandCategory {
    pub const fn title(self) -> &'static str {
        match self {
            ...
            CommandCategory::Random => "RANDOM VARIATES",
...
pub const HELP_INDEX_GROUPS: &[CommandIndexGroup] = &[
    ...
    CommandIndexGroup {
        ...
        entries: &[
            "rnormal  rlognormal  rskewnormal  rcauchy  rstudentt/rt  rchisq  rf",
            "rbeta  rgamma  rexponential  rweibull  rpareto  rpert  rtriangular",
            "rfrechet  rgumbel  rinversegaussian  rnig  runiform",
            "rbernoulli  rbinomial  rpoisson  rgeometric  rhypergeometric  rzeta  rzipf",
        ],
    },
    CommandIndexGroup {
...
pub const COMMAND_SPECS: &[CommandSpec] = &[
    ...
    CommandSpec {
        name: "rnormal",
        aliases: &[
            "rlognormal", "rskewnormal", "rcauchy", "rstudentt", "rt", "rchisq", "rf",
            "rbeta", "rgamma", "rexponential", "rweibull", "rpareto", "rpert",
            "rtriangular", "rfrechet", "rgumbel", "rinversegaussian", "rnig",
            "runiform", "rbernoulli", "rbinomial", "rpoisson", "rgeometric",
            "rhypergeometric", "rzeta", "rzipf",
        ],
        help: concat!(
            "Random variate generators (use inside generate df col = expr).\n",
            "All respect set_seed and produce one value per DataFrame row.\n\n",
            "Continuous:\n",
            "  rnormal() or rnormal(mu, sigma)          Normal\n",
            "  rlognormal(mu, sigma)                      Log-normal\n",
            "  rskewnormal(loc, scale, shape)             Skew-normal\n",
            "  rcauchy(loc, scale)                        Cauchy\n",
            "  rstudentt(df) or rt(df)                    Student's t\n",
            "  rchisq(k)                                  Chi-squared\n",
            "  rf(d1, d2)                                 F distribution\n",
            "  rbeta(alpha, beta)                         Beta\n",
            "  rgamma(shape, scale)                       Gamma\n",
            "  rexponential(lambda)                       Exponential (rate lambda)\n",
            "  rweibull(scale, shape)                     Weibull\n",
            "  rpareto(scale, shape)                      Pareto\n",
            "  rpert(min, max, mode) or rpert(..., shape) PERT\n",
            "  rtriangular(min, max, mode)                Triangular\n",
            "  rfrechet(location, scale, shape)           Frechet\n",
            "  rgumbel(location, scale)                   Gumbel\n",
            "  rinversegaussian(mean, shape)              Inverse Gaussian\n",
            "  rnig(alpha, beta)                          Normal-inverse-Gaussian\n",
            "  runiform() or runiform(a, b)               Uniform [0,1) or [a,b)\n\n",
            "Discrete:\n",
            "  rbernoulli(p)                              Bernoulli (default p=0.5)\n",
            "  rbinomial(n, p)                            Binomial\n",
            "  rpoisson(lambda)                           Poisson\n",
            "  rgeometric(p)                              Geometric (failures before first success)\n",
            "  rhypergeometric(N, K, n)                 Hypergeometric\n",
            "  rzeta(a)                                   Zeta (a > 1)\n",
            "  rzipf(N, s)                                Zipf\n\n",
            "Examples:\n",
            "  set_seed(42)\n",
