...
fn rd_children(r: &greeners::RdResult) -> Vec<(String, Value)> {
    let mut vars = Vec::new();
    let mut map = HashMap::new();
...
fn synth_children(r: &greeners::SynthResult) -> Vec<(String, Value)> {
    let mut vars = Vec::new();
...
fn synth_fit_dict(r: &greeners::SynthResult) -> Value {
    let mut entries = vec![
...
fn psm_children(r: &greeners::PsmResult) -> Vec<(String, Value)> {
    let mut vars = Vec::new();
    let mut map = HashMap::new();
...
fn did_children(r: &greeners::DidResult) -> Vec<(String, Value)> {
    regression_children(RegressionCtx {
        names: r.variable_names.clone(),
        params: &r.params,
        std_errors: &r.std_errors,
...
fn did_fit_dict(r: &greeners::DidResult) -> Value {
    fit_dict(&[
...
fn km_children(r: &greeners::KMResult) -> Vec<(String, Value)> {
    let mut vars = Vec::new();
    let n = r.times.len();
...
fn km_fit_dict(r: &greeners::KMResult) -> Value {
    fit_dict(&[
...
fn cox_children(r: &greeners::CoxResult) -> Vec<(String, Value)> {
    let mut vars = regression_children(RegressionCtx {
        names: r.variable_names.clone().unwrap_or_default(),
        params: &r.params,
        std_errors: &r.std_errors,
...
fn cox_fit_dict(r: &greeners::CoxResult) -> Value {
    fit_dict(&[
        ("log_lik", Value::Float(r.log_likelihood)),
...
fn heckman_children(r: &greeners::HeckmanResult) -> Vec<(String, Value)> {
    let mut vars = Vec::new();
...
fn heckman_fit_dict(r: &greeners::HeckmanResult) -> Value {
    fit_dict(&[
