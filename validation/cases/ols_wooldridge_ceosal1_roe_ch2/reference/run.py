# Reference implementation in Python/statsmodels for Wooldridge ceosal1,
...
if not CSV_PATH.exists():
    try:
        ...
        url = "https://raw.githubusercontent.com/vincentarelbundock/Rdatasets/master/csv/wooldridge/ceosal1.csv"
        df = pd.read_csv(url)
    df.to_csv(CSV_PATH, index=False)
else:
    df = pd.read_csv(CSV_PATH)

# Estimate the model from Chapter 2, Example 2.3.
model = smf.ols("salary ~ 1 + roe", data=df).fit()
