# Reference implementation in R for the book MA(1) case.
# Replicates Hayashi's default Hannan-Rissanen two-step estimator.

df <- read.csv("validation/cases/ma_book/data/ma.csv")
z <- df$y
t <- length(z)
p <- 0
q <- 1
...
# Step 2: regression y_t = c + ma1*u_hat_{t-1}.
start2 <- q
n_final <- n_long - start2

x_final <- matrix(1.0, nrow = n_final, ncol = 1 + q)
...
result <- list(
  coefficients = list(
    ...
    "ma.L1" = as.numeric(beta[2])
  ),
  standard_errors = list(
    intercept = as.numeric(se[1]),
    "ma.L1" = as.numeric(se[2])
  )
)

out_dir <- "validation/cases/ma_book/reference"
