# Reference implementation in R for the book AR(1) case.
# Replicates Hayashi's default Hannan-Rissanen two-step estimator.

df <- read.csv("validation/cases/ar_book/data/ar.csv")
z <- df$y
t <- length(z)
p <- 1
q <- 0
...
# Step 2: regression y_t = c + ar1*z_{t-1} (+ ma lags if q>0).
start2 <- max(q, 1)
n_final <- n_long - start2

x_final <- matrix(1.0, nrow = n_final, ncol = 1 + p)
...
out_dir <- "validation/cases/ar_book/reference"
