# Reference implementation in R for white test
...
data_dir <- "validation/cases/white_simulated/data"
dir.create(data_dir, recursive = TRUE, showWarnings = FALSE)
write.csv(df, file.path(data_dir, "data.csv"), row.names = FALSE)

m <- lm(y ~ x + z, data = df)

w <- bptest(m, ~ x*z + I(x^2) + I(z^2), data = df)
result <- list(
  lm_stat = as.numeric(w$statistic),
  p_value = as.numeric(w$p.value)
)

out_dir <- "validation/cases/white_simulated/reference"
