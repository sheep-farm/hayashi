# Reference implementation in Python/statsmodels for Wooldridge hprice2,
...
if not CSV_PATH.exists():
    try:
        ...
        url = "https://raw.githubusercontent.com/vincentarelbundock/Rdatasets/master/csv/wooldridge/hprice2.csv"
        df = pd.read_csv(url)
    df.to_csv(CSV_PATH, index=False)
else:
    df = pd.read_csv(CSV_PATH)

# Create log distance and rooms quadratic.
import numpy as np
df['ldist'] = np.log(df['dist'])
df['roomsq'] = df['rooms'] ** 2

# Estimate the model from Chapter 6, Example 6.2.
model = smf.ols("lprice ~ 1 + lnox + ldist + rooms + roomsq + stratio", data=df).fit()
