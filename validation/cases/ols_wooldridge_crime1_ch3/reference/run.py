# Reference implementation in Python/statsmodels for Wooldridge crime1,
...
if not CSV_PATH.exists():
    try:
        ...
        url = "https://raw.githubusercontent.com/vincentarelbundock/Rdatasets/master/csv/wooldridge/crime1.csv"
        df = pd.read_csv(url)
    df.to_csv(CSV_PATH, index=False)
else:
    df = pd.read_csv(CSV_PATH)

# Estimate the arrest records equation from Example 3.5.
model = smf.ols("narr86 ~ 1 + pcnv + ptime86 + qemp86", data=df).fit()
