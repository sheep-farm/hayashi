# Reference implementation in Python for the panel FE Grunfeld case.
...
if not CSV_PATH.exists():
    ...
    # Map firm names to integer IDs for Hayashi's panel engine.
    df["firm"] = pd.factorize(df["firm"])[0] + 1
    df.to_csv(CSV_PATH, index=False)
else:
    df = pd.read_csv(CSV_PATH)

# Entity-indexed panel for linearmodels.
panel_df = df.set_index(["firm", "year"])

# Fixed-effects investment demand model.
model = PanelOLS.from_formula("inv ~ value + capital + EntityEffects", data=panel_df).fit()
