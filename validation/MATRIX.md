...
| qreg | simulated | Python | pass | — | Simulated heteroskedastic data y = 1 + 2x + (1 + 0.5x) * (Exponential(1)-1). Quantile regression at tau=0.75 compared against statsmodels. boot=0 to avoid bootstrap overhead. |
...
| rf | simulated | Python | pass | — | Simulated data y = 3*x1 + N(0, 0.1). In-sample R² compared against scikit-learn RandomForestRegressor with the same tree and depth settings. |
...
| spatial_durbin | simulated | Python | pass | — | Data generated on a 7x7 grid with rook contiguity W, rho=0.3, beta=0.5, theta=0.2. Only rho is compared because the intercept and spatially lagged intercept are collinear in the cross-sectional specification. |
