...
\texttt{rlm} fits a linear model with M-estimators that down-weight outliers.
...
Use \texttt{rlm} when the data contain influential outliers but the majority of
the distribution is approximately normal.

\section{Generalised Linear Models}

\texttt{glm} fits a linear predictor through a link function. It generalises
...
Available families include \texttt{gaussian}, \texttt{poisson}, \texttt{binomial}, \texttt{gamma},
\texttt{inverse\_gaussian}, and \texttt{negative\_binomial}. Links include \texttt{identity},
\texttt{log}, \texttt{logit}, \texttt{probit}, \texttt{cloglog}, and \texttt{inverse}.
...
Correlation structures include \texttt{independence}, \texttt{exchangeable}, \texttt{ar1}, and
\texttt{unstructured}.

\section{Beta regression}

\texttt{betareg} is appropriate when the dependent variable is bounded in
$(0,1)$.
...
\texttt{gam} fits a generalised additive model with smoothing splines for
nonlinear effects.
...
\texttt{lowess} is a local polynomial smoother for visual exploration and
partial regression.
...
\texttt{mice} performs multiple imputation by chained equations. It fills in
