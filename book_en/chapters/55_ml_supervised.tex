...
OLS or logit, and they return model objects that work with `predict`,
`glance`, and `tidy`.
...
Use `predict(df, yhat = m_rf)` to obtain fitted values on a DataFrame.
...
The `mlp` command fits a single-hidden-layer feed-forward network with
...
For time-series, `lstm` and `transformer` use sequence windows to
...
predictive quality is convention-sensitive. They are experimental.
...
compatible with `predict` and `glance`. Standard errors are generally not
