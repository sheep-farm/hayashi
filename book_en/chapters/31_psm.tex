...
df |> filter(_n <= 1000)
generate df x1 = rnormal()                    // x1 ~ U(0,1)
generate df u  = rnormal()                    // assignment noise
...
════════════════════════════════════════════════════════════════════════
 ...
 k=1 match   Caliper: none   Bootstrap SE: 200 reps
 N treated: 503   N controls: 497   N matched treated: 497
────────────────────────────────────────────────────────────────────────
 ATT = 2.2168   SE = 0.0643   z = 34.483   P>|z| = 0.0000  ***
 95% CI: [2.0908, 2.3428]
────────────────────────────────────────────────────────────────────────
 Balance  x1:  μ_Treat=0.542  μ_Ctrl(raw)=0.461  μ_Ctrl(mtch)=0.461
                SMD_before=0.291   SMD_after=0.291
 (!) SMD > 0.10 after matching — poorly balanced covariate
════════════════════════════════════════════════════════════════════════
\end{lstlisting}

Without a caliper, matching accepts pairs that are distant in $\hat{p}$. Balance
does not improve (SMD = 0.291 before and after) and the estimate remains biased
($\hat\tau = 2{,}22$).
...
════════════════════════════════════════════════════════════════════════
 Outcome: y   Treatment: d
 k=1 match   Caliper: 0.0500   Bootstrap SE: 200 reps
 N treated: 503   N controls: 497   N matched treated: 449
────────────────────────────────────────────────────────────────────────
 ATT = 2.0323   SE = 0.0233   z = 87.157   P>|z| = 0.0000  ***
 95% CI: [1.9866, 2.0780]
────────────────────────────────────────────────────────────────────────
 Balance  x1:  μ_Treat=0.542  μ_Ctrl(raw)=0.461  μ_Ctrl(mtch)=0.485
                SMD_before=0.291   SMD_after=0.204
════════════════════════════════════════════════════════════════════════
\end{lstlisting}

With caliper $= 0{,}05$, the estimated ATT is $2{,}03$ — practically on the
true value. The cost is the loss of 54 treated units with no acceptable match
(503 → 449). The caliper reduces the SMD from 0.29 to 0.20, evidencing better
balance.\footnote{%
