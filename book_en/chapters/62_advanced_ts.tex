...
In `tvp` the coefficient vector is a random walk observed with noise:
...
`tvp` estimates the variances of $\varepsilon_t$ and $\eta_t$ by MLE and
...
Use `tvp_var` for a full vector autoregression with time-varying
coefficients.

\section{Threshold and Markov-switching VAR}

`setar` allows two AR regimes split by a threshold value of the lagged
dependent variable.
...
`msvar` allows the coefficients themselves to switch according to a
latent Markov chain.
...
`nardl` decomposes a regressor into positive and negative partial sums,
...
`pstr` does the same in a panel, with coefficients that change smoothly
...
`fcoef` estimates coefficients that vary smoothly with a moderator
...
`bvar` estimates a VAR with a Minnesota prior, shrinking own lags
...
`mfvar` combines low- and high-frequency variables using an exponential
...
`pvar` estimates a panel VAR using Arellano-Bond style GMM: lagged
