...
A block can be used as an expression. It evaluates a sequence of statements and returns the value of the last expression. Variables declared inside the block are local to it:

\begin{lstlisting}[caption={Block expression returning a value}]
let df = {
...
After the block, \lstinline{df} is available but \lstinline{raw} is gone.

\section{Output control}

\lstinline{quietly on} suppresses automatic output from statements and estimators. \lstinline{print(...)} and \lstinline{display ...} still appear. \lstinline{quietly off} restores normal output. The flag is scope-aware:

\begin{lstlisting}[caption={quietly on/off with scope}]
quietly on
...
Inside the block, \lstinline{quietly off} would temporarily enable output; it reverts to \lstinline{quietly on} when the block ends.
