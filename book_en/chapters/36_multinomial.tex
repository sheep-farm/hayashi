...
The Wooldridge \texttt{beauty} dataset is used in the validation cases
\texttt{ologit\_beauty} and \texttt{oprobit\_beauty} in \texttt{validation/cases/},
and the \texttt{AER::TravelMode} dataset is used in the \texttt{mlogit\_mode} case.
The cases estimate ordered logit, ordered probit and multinomial logit models in
\hayashi{} and compare the coefficients with reference implementations in R and
Python. Run \lstinline{hay validate} to see the current status.
