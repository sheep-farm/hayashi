...
outer environment (only send-safe values are captured). Auto-display is
suppressed inside threads --- only explicit \texttt{print()} calls
produce output. This has important consequences:
...
\begin{itemize}
  ...
  \item \textbf{Auto-display is suppressed}: the body's last expression
    is collected as a result, not printed. Use \texttt{print()}
    explicitly if you need output from individual iterations.
\end{itemize}
