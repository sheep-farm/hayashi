...
generate df y   = floor(lam + rnormal())
...
\noindent\textbf{Note on \texttt{rnormal()}:} in Hayashi, it generates $U(0,1)$, \emph{not} $N(0,1)$.
When replicating DGPs on other platforms, replace with \texttt{runiform()} (Stata),
\texttt{runif()} (R) or \texttt{np.random.uniform()} (Python).
