...
Robust Hausman  & \texttt{hausman\_robust()} & manual           & \texttt{phtest(robust)} & manual \\
...
\midrule
\multicolumn{5}{l}{\textit{Clustering and dimension reduction}} \\
K-Means         & \texttt{kmeans()}     & \texttt{cluster kmeans} & \texttt{kmeans()}    & \texttt{KMeans()} \\
Hierarchical    & \texttt{hclust()}     & \texttt{cluster single} & \texttt{hclust()}    & \texttt{AgglomerativeClustering()} \\
DBSCAN          & \texttt{dbscan()}     & —                     & \texttt{dbscan()}      & \texttt{DBSCAN()} \\
t-SNE           & \texttt{tsne()}       & —                     & \texttt{tsne()}        & \texttt{TSNE()} \\
UMAP            & \texttt{umap()}       & —                     & \texttt{umap()}        & \texttt{UMAP()} \\
PCA biplot      & \texttt{biplot()}     & \texttt{pca}          & \texttt{biplot()}      & manual \\
\midrule
\multicolumn{5}{l}{\textit{Post-estimation}} \\
Tidy coefs.     & \texttt{tidy()}       & —                     & \texttt{broom::tidy()} & \texttt{statsmodels summary} \\
