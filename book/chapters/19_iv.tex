...
// eps = resíduo de v sobre [1, z]: garante Z'eps = 0 exatamente
let d1 = cov(df, z, v) / variance(df, z)
let d0 = mean(df, v) - d1 * mean(df, z)
generate df eps = v - d0 - d1*z

generate df x = 0.8*z + eps        // regressor endógeno
...
de máquina. O OLS superestima $\beta_1$ em $+0{,}42$ (2.4233 vs.\ 2.0000),
